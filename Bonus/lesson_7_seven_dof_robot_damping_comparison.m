...
n  = 7; l = ones(n,1);
q0 = (pi/12)*ones(n,1);    % non-singular start
ut = [2; 1];

% Control params
eta = 10; tau = 0.01; T = 4.0; K = round(T/tau);
...
for k=1:K
    ...
    qdot = pinv(J) * (eta*e);
    un(k) = norm(qdot); q = q + tau*qdot;
end
errs{1}=en; uNorm{1}=un; names{1}='pinv (SVD)';

% DLS controllers
for j=1:numel(lset)
    lam=lset(j); q=q0; en=zeros(1,K); un=zeros(1,K);
    for k=1:K
        p=fk(q); e=ut-p; en(k)=norm(e);
        csum=cumsum(q); J=zeros(2,n);
        for i=1:n
    ...
    errs{1+j}=en; uNorm{1+j}=un; names{1+j}=sprintf('DLS \\lambda=%.3g',lam);
end

% Plots
mxK = max(cellfun(@numel, errs));
subplot(1,2,1); hold on; grid on;
for i=1:numel(errs), plot((0:mxK-1)*tau, errs{i}, 'LineWidth',1.5); end
xlabel('time [s]'); ylabel('||e_t||'); title('Task-space error norm');
legend(names,'Location','northeastoutside');

subplot(1,2,2); hold on; grid on;
for i=1:numel(uNorm), plot((0:mxK-1)*tau, uNorm{i}, 'LineWidth',1.5); end
